...
| `ArchitectMusic.cs` / music patches (four declarations added 2026-09-15, beyond the snapshot counts above) | `NRunMusicController.PlayCustomMusic`, `StopCustomMusic`, `StopMusic`; `CombatRoom.OnCombatEnded`, `Exit`; `NAudioManager.SetBgmVol` | Pre/Post | Exact-path routing of the Architect encounter to a room-owned OGG player, decoder-level looping, native BGM volume curve, and exit cleanup. **Medium/High**: relies on the music controller's `Proxy.stop_music` contract and the native Godot Master bus. Applies across both encounter phases without restarting. Handles raw OGG in quick PCKs and imported OGG resources in Godot exports; both need the music credits. |
...
| `WatcherStanceIcons` | `Resource.TakeOverPath`, strongly retained `AtlasTexture` aliases for the two `power_atlas.sprites/thearchitect-*_stance_power.tres` paths and `CompressedTexture2D` aliases for `images/enchantments/thearchitect-{wrath,calm}.png` | **Medium** native path/type contract. Both native fallback and BaseLib custom paths must resolve the same original pixels, even when nonvirtual native getters are inlined. Enchantment tooltip loading requires a compressed texture. No vanilla resource aliases are replaced. |
