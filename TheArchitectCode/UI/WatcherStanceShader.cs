                ...
                if (wrath) {
                    for (int i = 0; i < 18; i++) {
                        ...
                        if (front == front_layer) {
                            float x = (fract(seed * 0.618034) - 0.5) * 1.42;
                            ...
                            float d = p.x - x - sway;
                            light += glow(d, 0.028) * tip * fade * 0.7;
                            core += glow(d, 0.005) * tip * fade;
                        }
                    }
                    if (!front_layer) {
                        float pulse = 0.78 + 0.22 * sin(t * 3.6);
                        float flame = 0.82 + 0.18 * sin(p.x * 29.0 + t * 2.6 + sin(p.y * 12.0 - t * 4.0));
                        mist = glow(p.x, 0.49) * glow(p.y - 0.62, 0.5) * pulse * flame * 0.34;
                        light += glow(p.x, 0.62) * glow(p.y - 0.99, 0.055) * pulse * 0.5;
                    }
                } else {
                    for (int i = 0; i < 7; i++) {
                        float seed = float(i);
                        ...
                        float rx = 0.57 + 0.10 * sin(seed * 2.1 + t * 0.4);
                        float ry = 0.043;
                    ...
                    if (!front_layer) {
                        ...
                        mist = glow(p.x, 0.50) * glow(p.y - 0.66, 0.50) * breath * 0.17;
                        light += glow(p.x, 0.69) * glow(p.y - 0.99, 0.05) * breath * 0.23;
                    }
                }
